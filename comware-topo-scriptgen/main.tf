...
resource "coder_script" "create_gns3_project" {
  ...
  script = <<EOF
    ...
    echo -e "\033[36m- 📄 Writing '/tmp/create_gns3_project.py'...\033[0m"
    echo "${filebase64("${path.module}/assets/create_gns3_project.py")}" | base64 -d > /tmp/create_gns3_project.py
    python3 /tmp/create_gns3_project.py --project-name "topo-scriptgen--${local.workspace}--${local.username}" --project-id-file "/home/${local.username}/.gns3_project_id"
  EOF
}
