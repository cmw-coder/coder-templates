...
## 重要提示：优先使用 -uko 参数
在提交abuild编译任务时，**强烈建议优先使用 -uko 参数**：
- `-u`: 编译用户态代码
- `-k`: 编译内核态代码
- `-o`: 编译obj文件（包括用户态和内核态）

组合参数 `-uko` 能够同时编译用户态和内核态的obj文件，对于sim版本编译速度较快，是推荐的默认编译参数。

## 快速开始
### 如何通过abuild编译结果，修改错误代码
1. 在Linux的bash终端键入abuild命令，**优先使用 -uko 参数**，例如‘abuild -e 64sim7cen -uko’。这个参数组合会同时编译用户态和内核态的obj文件，编译sim版本很快。
...
### 提交abuild编译任务
```bash
# 基本用法（必须指定 IPE 包名），优先使用 -uko 参数
abuild -e <ipe_package_name> -uko
# 示例：编译单个包，使用 -uko 参数（推荐）
abuild -e 64sim7cen -uko
