...
resource "coder_script" "install_find_skills" {
  agent_id     = coder_agent.main.id
  display_name = "Install find-skills"
  icon         = "/emojis/1f50d.png"
  run_on_start = true
  script = <<EOF
    #!/bin/bash
    SKILL_DIR="/home/${local.username}/.agents/skills/find-skills"
    if [ -f "$SKILL_DIR/SKILL.md" ]; then
      echo -e "\033[36m- find-skills is already installed.\033[0m"
      exit 0
    fi
    echo -e "\033[36m- Installing find-skills...\033[0m"
    npx -y skills add vercel-labs/skills@find-skills -g -y
    echo -e "\033[32m- find-skills installed!\033[0m"
  EOF
}
