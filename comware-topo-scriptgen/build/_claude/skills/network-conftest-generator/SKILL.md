...
### 步骤 0：意图检测
**目标**：在执行任何操作前，判断用户是否明确表示不需要生成或修改 `conftest.py`。


1. **分析用户输入**：
   仔细阅读用户的原始请求，识别是否存在以下**明确的否定意图**：
   - 明确表示"不需要"、"不要"、"不用"、"不生成"、"不修改"、"跳过"、"忽略"等否定词汇
   - 这些否定词汇与 `conftest.py`、测试背景、环境配置等关键词相关联

2. **判定标准**：
   **用户明确表示不需要 conftest.py 的情况包括但不限于**：
   - "不要修改 conftest.py"
   - "不需要生成 conftest.py"
   - "跳过 conftest.py 的生成"
   - "只关注测试用例，不需要修改 conftest"
   - "conftest.py 已经配置好了，不用动"
   - "不涉及 conftest.py 的修改"
   ...
   **需要继续执行的情况包括但不限于**：
   ...
   - "设置测试背景"
   - "更新 conftest.py"
   - 未提及 conftest.py，但需要配置测试环境/背景
   - 用户仅描述测试需求，未明确排除 conftest.py 生成

3. **执行决策**：
   - **情况 A：检测到明确的否定意图**
     - **动作**：停止执行后续所有步骤
     - **反馈**：回复"检测到您明确表示不需要修改/生成 conftest.py，已跳过环境配置流程。"
     - **结束**：不执行任何文件操作或检索操作

   - **情况 B：未检测到否定意图或用户意图不明确**
     - **动作**：继续执行步骤 1
     - **说明**：默认为需要检查或生成 conftest.py

4. **注意事项**：
   - **优先级最高**：此步骤优先于所有其他步骤，无论任何情况都必须先执行意图检测
   - **保守判定**：当用户意图不明确时，默认继续执行流程（宁可多做检查，也不要遗漏必要的配置）
   - **仅处理明确否定**：只有在用户**明确表述**否定意图时才跳过，推测性的意图不作为判定依据
...
7. 接口使用注意实现事项：
   ...
   - 如果业务需要在二层口下进行操作: gl.dut.port1.l2intf    -------  例如端口下添加vlan，或进行lldp等二层业务配置
   - 如果业务需要在三层层口下进行操作：gl.dut.port1.l3intf    -----  返回对应的三层口GigbitEthernet1/0/1（如果是三层口的话）, Vlan-Interface100
