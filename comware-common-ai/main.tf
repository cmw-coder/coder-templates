...
# =============================================================================
# HTTP data sources: pre-fetched SVN directory maps
# =============================================================================

# Fetch pre-computed platform SVN tree from the host-side cron job.
# The JSON files are generated by update-svn-maps.sh (which calls
# get-svn-all-map.sh) and served as static files by Caddy on the Docker host.
# This replaces the slow on-demand SVN traversal that previously ran during
# every terraform plan (~4 minutes per repo).
#
# Prerequisites:
#   1. update-svn-maps.sh cron job running on the Docker host
#   2. Caddy serving /opt/coder-svn-maps/ on the configured port
data "http" "platform_map" {
  url                = "${local.svn_maps_base_url}/platform_map.json"
...
data "http" "public_map" {
  url                = "${local.svn_maps_base_url}/public_map.json"
  request_timeout_ms = 10000
...
locals {
  # SVN maps HTTP endpoint (served by Caddy on the Docker host).
  svn_maps_base_url = "https://assets.coder.h3c.com"
  ...
  # Pre-fetched COMPLETE SVN directory maps (served as static JSON by Caddy).
  # Structure: { "V9R1": { "branches": [...], "trunk": { "depth": 0, "folders": [...] }, ... }, ... }
  platform_map = jsondecode(data.http.platform_map.response_body)
  public_map   = jsondecode(data.http.public_map.response_body)
