...
def add_fingerprint_to_file(file_path, uuid, is_update=False):
    ...
    try:
        ...
        # 由于类名是强制已知的"TestClass"，使用更简单直接的匹配方式
        
        # 查找精确的类定义：class TestClass:
        test_class_pattern = r'class\s+TestClass\s*:'
        ...
        if class_match:
            ...
            if all_matches:
                ...
                if project_info_match:
                    ...
                    # 在项目详细信息END标记前插入UUID
                    fingerprint_line = f' AI_FingerPrint_UUID: {uuid}\n'
        ...
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(fingerprint_comment + content)
            if is_update:
                print(f"成功为 {file_path} 更新AI指纹: {uuid}")
            else:
                print(f"成功为 {file_path} 添加AI指纹: {uuid}")
