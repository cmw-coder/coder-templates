...
eval "$(jq -r '@sh "PRESS_SVN_URL_INPUT=\(.press_svn_url) SVN_PASSWORD_INPUT=\(.svn_password) SVN_USERNAME_INPUT=\(.svn_username)"')"

# Get configuration from environment variables, use defaults as fallback
PRESS_SVN_URL=${PRESS_SVN_URL_INPUT:-${PRESS_SVN_URL:-"http://10.153.3.214/comware-test-script/50.多环境移植/1/AIGC/INFO/"}}
