...
class CVarsAndFuncs:
    '''
    测试背景：
    3台设备组网，DUT1、DUT2建立ISIS 1的邻居，DUT2和DUT3建立ISIS 1的邻居，三台设备均配置成L2，在DUT3上配置一个loopback，loopback下配置一条ipv6路由3::3，使能isis，在DUT1上创建loopback,配置地址1.1.1.9,使能isisv6，DUT2上创建loopback，配置地址2.2.2.9，使能isis，DUT1和DUT2之间建立BGP ipv4邻居，DUT2和DUT3之间建立BGP ipv4邻居。
    '''
    pass
...
@atf_adornment
def setup():
    #在DUT2上启动ISIS进程，接口使能ISIS，DUT2创建loopback，和DUT1、DUT3建立IBGP邻居
    gl.DUT2.send(f'''
            ctrl+z
            system-view
            isis 1
			...
			peer 3.3.3.3 enable
            ''')
    #在DUT1上启动ISIS进程，接口使能ISIS，DUT1创建loopback，和DUT2建立IBGP邻居
    gl.DUT1.send(f'''
            ctrl+z
            system-view
            isis 1
			...
			peer 2.2.2.9 enable
            ''')
    #在DUT3上启动ISIS进程，接口使能ISIS,DUT3创建loopback，和DUT2建立IBGP邻居
    gl.DUT3.send(f'''
            ctrl+z
            system-view
            isis 1
