...
1. **自动检测域账号**（通过 `whoami`）。
2. **配置环境变量**（使用路由红区预配置的 API Key 和端点）。
3. **保存配置** 到 `~/.bashrc`（API Key、部门、模型设置等）。
4. **启动 Claude Code**（IDE 模式，已预配置权限）。
...
| 变量名                            | 说明                        |
| --------------------------------- | --------------------------- |
| `ANTHROPIC_API_KEY`              | 路由红区 API Key（预配置）  |
...
| `H3C_DEPARTMENT`                 | 部门（Routing&MPLS）        |
| `H3C_USERNAME`                   | 你的域账号                  |
